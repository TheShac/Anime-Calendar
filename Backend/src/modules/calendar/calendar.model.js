...
export async function findPublicCalendarBySeason(seasonId) {
  return await db
    .select({
      entryId: calendarEntries.id, dayOfWeek: calendarEntries.dayOfWeek,
      time: calendarEntries.time, animeId: animes.id, title: animes.title,
    ...
    .innerJoin(animes,  eq(calendarEntries.animeId,  animes.id))
    .innerJoin(seasons, eq(calendarEntries.seasonId, seasons.id))
    .where(eq(calendarEntries.seasonId, seasonId));
}

export async function findAllCalendarEntries() {
  return await db
    .select({
      id: calendarEntries.id, dayOfWeek: calendarEntries.dayOfWeek,
      time: calendarEntries.time, animeId: animes.id, animeTitle: animes.title,
      animeImage: animes.imageUrl, seasonId: seasons.id, seasonName: seasons.name,
    })
    .from(calendarEntries)
...
export async function updateCalendarEntryById(id, data) {
  const [entry] = await db.update(calendarEntries).set(data).where(eq(calendarEntries.id, id)).returning();
  return entry;
}

export async function deleteCalendarEntryById(id) {
  const [entry] = await db.delete(calendarEntries).where(eq(calendarEntries.id, id)).returning();
  return entry;
}